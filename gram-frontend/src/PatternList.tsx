...
const PatternList: React.FC<PatternListProps> = ({ patterns, anchorLabel, onSelectedPatternsChange, onCombinePatterns, sessionId }) => {
  ...
  const getPatternStructure = (pattern: Pattern) => {
    ...
    const relTypes = pattern.rel_types || [];

    if (pattern.kind === 'anyrel') {
      const relType = `*ANYREL*${relTypes.length > 0 ? `:${relTypes.length}` : ''}`;
      structure.push(
      ...
      for (let i = 0; i < relTypes.length; i++) {
        const relType = relTypes[i] || 'ANYREL';
        const targetNodeLabel = pattern.node_labels[i + 1];
        const isFinalNode = i === relTypes.length - 1;
